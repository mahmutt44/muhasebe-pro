# Çoklu dil desteği - Türkçe ve Arapça

TRANSLATIONS = {
    'tr': {
        # Genel
        'app_name': 'Muhasebe Pro',
        'dashboard': 'Ana Sayfa',
        'customers': 'Müşteriler',
        'products': 'Ürünler',
        'receipts': 'Fişler',
        'transactions': 'İşlemler',
        'settings': 'Ayarlar',
        'language': 'Dil',
        'turkish': 'Türkçe',
        'arabic': 'العربية',
        
        # Dashboard
        'welcome': 'Hoş Geldiniz',
        'total_customers': 'Toplam Müşteri',
        'total_products': 'Toplam Ürün',
        'total_receipts': 'Toplam Fiş',
        'total_income': 'Toplam Gelir',
        'total_expense': 'Toplam Gider',
        'net_balance': 'Net Bakiye',
        'current_balance': 'Güncel bakiye',
        'all_time': 'Tüm zamanlar',
        'total_receivable': 'Toplam alacak',
        'quick_actions': 'Hızlı İşlemler',
        'new_customer': 'Yeni Müşteri',
        'new_product': 'Yeni Ürün',
        'new_receipt': 'Yeni Fiş',
        'new_transaction': 'Yeni İşlem',
        
        # Müşteriler
        'customer_name': 'Müşteri Adı',
        'company_name': 'İşletme Adı',
        'city': 'Şehir',
        'phone': 'Telefon',
        'notes': 'Notlar',
        'balance': 'Bakiye',
        'add_customer': 'Müşteri Ekle',
        'edit_customer': 'Müşteri Düzenle',
        'delete_customer': 'Müşteri Sil',
        'customer_details': 'Müşteri Detayları',
        'debtor': 'Borçlu',
        'debtor_customers': 'Borçlu Müşteri',
        'creditor': 'Alacaklı',
        'balance_clear': 'Temiz',
        'clear_customers': 'Temiz Müşteri',
        'total_debt': 'Toplam Borç',
        'customer_list': 'Müşteri Listesi',
        'search_customer': 'Müşteri ara...',
        
        # Ürünler
        'product_name': 'Ürün Adı',
        'unit': 'Birim',
        'unit_price': 'Birim Fiyat',
        'add_product': 'Ürün Ekle',
        'edit_product': 'Ürün Düzenle',
        'delete_product': 'Ürün Sil',
        'product_list': 'Ürün Listesi',
        'average_price': 'Ortalama Fiyat',
        'search_product': 'Ürün ara...',
        
        # Fişler
        'receipt_no': 'Fiş No',
        'date': 'Tarih',
        'total': 'Toplam',
        'subtotal': 'Ara Toplam',
        'tax': 'KDV',
        'tax_included': 'KDV Dahil',
        'tax_rate': 'KDV Oranı',
        'grand_total': 'Genel Toplam',
        'add_receipt': 'Fiş Oluştur',
        'receipt_details': 'Fiş Detayları',
        'select_customer': 'Müşteri Seçin',
        'select_product': 'Ürün Seçin',
        'quantity': 'Miktar',
        'price': 'Fiyat',
        'add_item': 'Ürün Ekle',
        'items': 'Ürünler',
        'print': 'Yazdır',
        'delete_receipt': 'Fiş Sil',
        'customer_balance': 'Müşteri Bakiyesi',
        'created_at': 'Oluşturulma',
        'receipt_items': 'Fiş Kalemleri',
        'receipt_summary': 'Fiş Özeti',
        'custom_price_hint': 'Özel fiyat için fiyat alanını değiştirebilirsiniz',
        'sales_receipt': 'SATIŞ FİŞİ',
        'customer': 'Müşteri',
        'customer_detail': 'Müşteri Detay',
        'product': 'Ürün',
        'product_count': 'Ürün Sayısı',
        'total_balance': 'Toplam Bakiye',
        'view_details': 'Detay Görüntüle',
        'customer_total_balance': 'Müşteri Toplam Bakiyesi',
        
        # İşlemler
        'transaction_type': 'İşlem Türü',
        'income': 'Gelir',
        'expense': 'Gider',
        'debt': 'Borç',
        'payment': 'Ödeme',
        'amount': 'Tutar',
        'description': 'Açıklama',
        'add_transaction': 'İşlem Ekle',
        'edit_transaction': 'İşlem Düzenle',
        'delete_transaction': 'İşlem Sil',
        'transaction_list': 'İşlem Listesi',
        'search_transaction': 'İşlem ara...',
        'income_expense': 'Gelir/Gider',
        
        # Butonlar
        'save': 'Kaydet',
        'cancel': 'İptal',
        'delete': 'Sil',
        'edit': 'Düzenle',
        'close': 'Kapat',
        'search': 'Ara',
        'filter': 'Filtrele',
        'all': 'Tümü',
        'back': 'Geri',
        'view': 'Görüntüle',
        'actions': 'İşlemler',
        'add': 'Ekle',
        'reset': 'Sıfırla',
        
        # Mesajlar
        'confirm_delete': 'Silmek istediğinize emin misiniz?',
        'success': 'Başarılı',
        'error': 'Hata',
        'no_data': 'Veri bulunamadı',
        'loading': 'Yükleniyor...',
        
        # Bildirim Mesajları
        'transaction_added': 'İşlem eklendi',
        'transaction_updated': 'İşlem güncellendi',
        'transaction_deleted': 'İşlem silindi',
        'customer_added': 'Müşteri eklendi',
        'customer_updated': 'Müşteri güncellendi',
        'customer_deleted': 'Müşteri silindi',
        'product_added': 'Ürün eklendi',
        'product_updated': 'Ürün güncellendi',
        'product_deleted': 'Ürün silindi',
        'product_removed': 'Ürün kaldırıldı',
        'receipt_created': 'Fiş oluşturuldu',
        'receipt_deleted': 'Fiş silindi',
        'error_occurred': 'Bir hata oluştu',
        'logout_success': 'Başarıyla çıkış yaptınız.',
        'user_created': 'Kullanıcı {username} başarıyla oluşturuldu.',
        'password_reset': 'Kullanıcı {username} şifresi sıfırlandı.',
        'select_product_error': 'Lütfen bir ürün seçin',
        'invalid_quantity_error': 'Lütfen geçerli bir miktar girin',
        'invalid_price_error': 'Lütfen geçerli bir fiyat girin',
        'add_product_error': 'Lütfen en az bir ürün ekleyin',
        
        # Onay Mesajları
        'confirm_delete_transaction': 'Bu işlemi silmek istediğinizden emin misiniz?',
        'confirm_delete_customer': 'Bu müşteriyi silmek istediğinizden emin misiniz?',
        'confirm_delete_product': 'Bu ürünü silmek istediğinizden emin misiniz?',
        'confirm_delete_receipt': 'Bu fişi silmek istediğinizden emin misiniz? Bu işlem geri alınamaz.',
        'confirm_clear_form': 'Formu temizlemek istediğinizden emin misiniz?',
        'confirm_delete_user': '{username} kullanıcısını silmek istediğinizden emin misiniz? Bu işlem geri alınamaz.',
        'delete_user': 'Kullanıcı Sil',
        'user_deleted': '{username} kullanıcısı silindi',
        'cannot_delete_self': 'Kendi hesabınızı silemezsiniz.',
        'admin_required': 'Bu işlem için admin yetkisi gerekiyor.',
        
        # Giriş/Kullanıcı
        'login': 'Giriş Yap',
        'logout': 'Çıkış Yap',
        'username': 'Kullanıcı Adı',
        'password': 'Şifre',
        'email': 'E-posta',
        'remember_me': 'Beni hatırla',
        'admin': 'Admin',
        'platform_admin': 'Platform Admin',
        'observer': 'Gözlemci',
        'admin_panel': 'Admin Paneli',
        'platform_admin_panel': 'Platform Admin Paneli',
        'company_admin_panel': 'Şirket Admin Paneli',
        'user_management': 'Kullanıcı Yönetimi',
        'users': 'Kullanıcılar',
        'role': 'Rol',
        'status': 'Durum',
        'active': 'Aktif',
        'inactive': 'Pasif',
        'add_new_user': 'Yeni Kullanıcı Ekle',
        'reset_password': 'Şifre Sıfırla',
        'new_password': 'Yeni Şifre',
        'cancel': 'İptal',
        'never': 'Hiç giriş yapmadı',
        'you': 'Siz',
        'users_list': 'Kullanıcı Listesi',
        'request_account': 'İşletme Hesabı Talep Et',
        'request_account_subtitle': 'Yeni bir işletme hesabı oluşturmak için talep gönderin',
        'business_info': 'İşletme Bilgileri',
        'business_type': 'İşletme Türü',
        'select_option': 'Seçiniz...',
        'cafe': 'Kafe',
        'market': 'Market',
        'restaurant': 'Restoran',
        'general': 'Genel',
        'authorized_person': 'Yetkili Adı',
        'short_note': 'Kısa Not',
        'short_note_placeholder': 'İşletme hakkında kısa bilgi...',
        'submit_request': 'Talep Gönder',
        'account_requests': 'Hesap Talepleri',
        'account_requests_subtitle': 'İşletme hesabı taleplerini yönetin',
        'request_list': 'Talep Listesi',
        'request_date': 'Talep Tarihi',
        'pending': 'Beklemede',
        'approved': 'Onaylandı',
        'rejected': 'Reddedildi',
        'approve': 'Onayla',
        'reject': 'Reddet',
        'no_requests': 'Henüz talep bulunmuyor',
        'approve_request_title': 'Talebi Onayla',
        'approve_request_confirm': '{company} talebini onaylamak istediğinizden emin misiniz?',
        'approve_request_info': 'Onaylandığında yeni bir işletme kaydı ve admin kullanıcısı oluşturulacaktır.',
        'reject_request_title': 'Talebi Reddet',
        'rejection_reason': 'Reddetme Sebebi',
        'rejection_reason_placeholder': 'Reddetme sebebinizi belirtin...',
        'request_already_processed': 'Bu talep daha önce işlenmiş.',
        'temporary_password': 'Geçici Şifre',
        'fill_required_fields': 'Lütfen zorunlu alanları doldurun.',
        'request_submitted': 'Hesap talebiniz başarıyla alındı.',
        'request_approved': 'Hesap talebi onaylandı.',
        'request_rejected': 'Hesap talebi reddedildi.',
        'login_credentials': 'Giriş bilgileri',
        
        # Platform Admin Panel
        'platform_admin_dashboard': 'Platform Admin Paneli',
        'global_system_management': 'Global Sistem Yönetimi',
        'total_companies': 'Toplam İşletme',
        'total_company_users': 'Şirket Kullanıcıları',
        'platform_admins': 'Platform Adminler',
        'pending_requests': 'Bekleyen Talepler',
        'recent_companies': 'Son Eklenen İşletmeler',
        'quick_actions': 'Hızlı İşlemler',
        'manage_companies': 'İşletmeleri Yönet',
        'all_companies': 'Tüm İşletmeler',
        'manage_all_companies': 'Sistemdeki tüm işletmeleri yönetin',
        'no_companies_yet': 'Henüz işletme bulunmuyor',
        'view_all': 'Tümünü Gör',
        'back': 'Geri',
        'add_platform_admin': 'Yeni Platform Admin Ekle',
        'platform_admin_management_desc': 'Platform seviyesi yöneticileri (işletmeden bağımsız)',
        'no_platform_admins': 'Henüz platform admin bulunmuyor',
        
        # İşletme Yönetimi
        'platform_management': 'Platform Yönetimi',
        'suspended': 'Askıda',
        'suspend': 'Askıya Al',
        'activate': 'Aktif Et',
        'delete': 'Sil',
        'view_details': 'Detay',
        'delete_company': 'İşletme Sil',
        'delete_company_confirm': 'işletmesini ve TÜM verilerini silmek istediğinizden emin misiniz?',
        'delete_company_warning': 'Bu işlem geri alınamaz! Tüm kullanıcılar, müşteriler, işlemler ve fişler silinecektir.',
        'company_details': 'İşletme Detay',
        'company_info': 'İşletme Bilgileri',
        'company_users': 'İşletme Kullanıcıları',
        'suspend_company': 'İşletmeyi Askıya Al',
        'activate_company': 'İşletmeyi Aktif Et',
        'edit_company': 'İşletme Düzenle',
        'save': 'Kaydet',
        'no_users': 'Kullanıcı bulunmuyor',
        'pending_requests_count': 'Bekleyen Talepler',
        'contact_info': 'İletişim',
        'request_details': 'Talep Detayları',
        'close': 'Kapat',
        'delete_request': 'Talebi Sil',
        'delete_request_confirm': 'talebini tamamen silmek istediğinizden emin misiniz?',
        'delete_request_warning': 'Bu işlem geri alınamaz!',
        
        'supplier_added': 'Tedarikçi eklendi',
        'supplier_updated': 'Tedarikçi güncellendi',
        'supplier_deleted': 'Tedarikçi silindi',
        
        # Tedarikçi
        'suppliers': 'Tedarikçiler',
        'suppliers_subtitle': 'Tedarikçi yönetimi ve borç takibi',
        'new_supplier': 'Yeni Tedarikçi',
        'edit_supplier': 'Tedarikçi Düzenle',
        'delete_supplier': 'Tedarikçiyi Sil',
        'supplier_name': 'Firma Adı',
        'company_name': 'Firma Adı',
        'contact_person': 'Yetkili Kişi',
        'total_suppliers': 'Toplam Tedarikçi',
        'debtor_suppliers': 'Borçlu Tedarikçi',
        'clear_suppliers': 'Borcu Olmayan',
        'total_debt': 'Toplam Borç',
        'search_supplier': 'Tedarikçi ara...',
        'no_suppliers': 'Henüz tedarikçi kaydı bulunmuyor.',
        'tax_number': 'Vergi Numarası',
        'address': 'Adres',
        'passive': 'Pasif',
        'confirm_delete_supplier': 'Bu tedarikçiyi silmek istediğinizden emin misiniz?',
        'supplier_info': 'Tedarikçi Bilgileri',
        'supplier_transactions': 'Tedarikçi İşlemleri',
        'total_purchases': 'Toplam Alış',
        'total_payments': 'Toplam Ödeme',
        'remaining_debt': 'Kalan Borç',
        
        # Raporlar
        'reports': 'Raporlar',
        'reports_subtitle': 'İşletmeniz için detaylı raporlar ve analizler',
        'view_report': 'Raporu Görüntüle',
        'profit_loss_report': 'Kar/Zarar Raporu',
        'profit_loss_desc': 'Aylık gelir, gider ve net kar analizi',
        'top_products_report': 'En Çok Satılan Ürünler',
        'top_products_desc': 'Satış performansı en yüksek ürünler',
        'customer_debts_report': 'Müşteri Borçları',
        'customer_debts_desc': 'Borçlu müşteriler ve bakiye durumu',
        'stock_report': 'Stok Raporu',
        'stock_report_desc': 'Mevcut stok ve düşük stok uyarıları',
        'monthly_trend': 'Aylık Trend (Son 12 Ay)',
        'income_transactions': 'Gelir İşlemleri',
        'expense_transactions': 'Gider İşlemleri',
        'no_income_transactions': 'Gelir işlemi bulunmuyor',
        'no_expense_transactions': 'Gider işlemi bulunmuyor',
        'total_sales_quantity': 'Toplam Satış Adedi',
        'sales_by_product': 'Ürünlere Göre Satış',
        'sales_by_unit': 'Birim Dağılımı',
        'top_selling_products': 'En Çok Satılan Ürünler Listesi',
        'quantity_sold': 'Satılan Adet',
        'no_sales_data': 'Henüz satış verisi bulunmuyor',
        'total_debtors': 'Borçlu Müşteri',
        'remaining_debt': 'Kalan Borç',
        'debt_distribution': 'Borç Dağılımı',
        'debt_vs_payment': 'Borç vs Ödeme',
        'paid': 'Ödenen',
        'remaining': 'Kalan',
        'high_risk': 'Yüksek Risk',
        'medium_risk': 'Orta Risk',
        'low_risk': 'Düşük Risk',
        'no_debt_customers': 'Tüm müşterilerin borcu ödenmiş',
        'low_stock_products': 'Düşük Stok Ürünü',
        'total_stock_value': 'Toplam Stok Değeri',
        'low_stock_alert': 'Düşük Stok Uyarısı!',
        'products_need_restock': 'ürün stok takviyesi gerektiriyor',
        'view_low_stock': 'Düşük Stokları Gör',
        'stock_details': 'Stok Detayları',
        'current_stock': 'Mevcut Stok',
        'threshold': 'Eşik Değer',
        'normal': 'Normal',
        'low': 'Düşük',
        'no_low_stock_products': 'Düşük stokta ürün bulunmuyor',
        'filter': 'Filtrele',
        'date': 'Tarih',
        'description': 'Açıklama',
        'revenue': 'Gelir',
        'status': 'Durum',
        
        # İşlemler sayfası
        'this_month': 'Bu Ay',
        
        # Rapor anahtarları
        'net_profit': 'Net Kar',
        'top_products_subtitle': 'Satış performansı en yüksek ürünler',
        'total_revenue': 'Toplam Gelir',
        'customer_debts_subtitle': 'Borçlu müşteriler ve bakiye durumu',
        'total_paid': 'Toplam Ödenen',
        'stock_report_subtitle': 'Mevcut stok ve düşük stok uyarıları',
        'show_low_stock_only': 'Sadece Düşük Stokları Göster',
    },
    
    'ar': {
        # عام
        'app_name': 'محاسبة برو',
        'dashboard': 'الرئيسية',
        'customers': 'العملاء',
        'products': 'المنتجات',
        'receipts': 'الفواتير',
        'transactions': 'المعاملات',
        'settings': 'الإعدادات',
        'language': 'اللغة',
        'turkish': 'Türkçe',
        'arabic': 'العربية',
        
        # لوحة التحكم
        'welcome': 'مرحباً',
        'total_customers': 'إجمالي العملاء',
        'total_products': 'إجمالي المنتجات',
        'total_receipts': 'إجمالي الفواتير',
        'total_income': 'إجمالي الدخل',
        'total_expense': 'إجمالي المصروفات',
        'net_balance': 'الرصيد الصافي',
        'current_balance': 'الرصيد الحالي',
        'all_time': 'كل الأوقات',
        'total_receivable': 'إجمالي المستحقات',
        'quick_actions': 'إجراءات سريعة',
        'new_customer': 'عميل جديد',
        'new_product': 'منتج جديد',
        'new_receipt': 'فاتورة جديدة',
        'new_transaction': 'معاملة جديدة',
        
        # العملاء
        'customer_name': 'اسم العميل',
        'company_name': 'اسم المنشأة',
        'city': 'المدينة',
        'phone': 'الهاتف',
        'notes': 'ملاحظات',
        'balance': 'الرصيد',
        'add_customer': 'إضافة عميل',
        'edit_customer': 'تعديل العميل',
        'delete_customer': 'حذف العميل',
        'customer_details': 'تفاصيل العميل',
        'debtor': 'مدين',
        'debtor_customers': 'عملاء مدينون',
        'creditor': 'دائن',
        'balance_clear': 'صافي',
        'clear_customers': 'عملاء بدون ديون',
        'total_debt': 'إجمالي الديون',
        'customer_list': 'قائمة العملاء',
        'search_customer': 'بحث عن عميل...',
        
        # المنتجات
        'product_name': 'اسم المنتج',
        'unit': 'الوحدة',
        'unit_price': 'سعر الوحدة',
        'add_product': 'إضافة منتج',
        'edit_product': 'تعديل المنتج',
        'delete_product': 'حذف المنتج',
        'product_list': 'قائمة المنتجات',
        'average_price': 'متوسط السعر',
        'search_product': 'بحث عن منتج...',
        
        # الفواتير
        'receipt_no': 'رقم الفاتورة',
        'date': 'التاريخ',
        'total': 'المجموع',
        'subtotal': 'المجموع الفرعي',
        'tax': 'الضريبة',
        'tax_included': 'شامل الضريبة',
        'tax_rate': 'نسبة الضريبة',
        'grand_total': 'المجموع الكلي',
        'add_receipt': 'إنشاء فاتورة',
        'receipt_details': 'تفاصيل الفاتورة',
        'select_customer': 'اختر العميل',
        'select_product': 'اختر المنتج',
        'quantity': 'الكمية',
        'price': 'السعر',
        'add_item': 'إضافة منتج',
        'items': 'المنتجات',
        'print': 'طباعة',
        'delete_receipt': 'حذف الفاتورة',
        'customer_balance': 'رصيد العميل',
        'created_at': 'تاريخ الإنشاء',
        'receipt_items': 'بنود الفاتورة',
        'receipt_summary': 'ملخص الفاتورة',
        'custom_price_hint': 'يمكنك تغيير السعر للسعر المخصص',
        'sales_receipt': 'فاتورة مبيعات',
        'customer': 'العميل',
        'customer_detail': 'تفاصيل العميل',
        'product': 'المنتج',
        'product_count': 'عدد المنتجات',
        'total_balance': 'الرصيد الإجمالي',
        'view_details': 'عرض التفاصيل',
        'customer_total_balance': 'الرصيد الإجمالي للعميل',
        
        # المعاملات
        'transaction_type': 'نوع المعاملة',
        'income': 'دخل',
        'expense': 'مصروف',
        'debt': 'دين',
        'payment': 'دفعة',
        'amount': 'المبلغ',
        'description': 'الوصف',
        'add_transaction': 'إضافة معاملة',
        'edit_transaction': 'تعديل المعاملة',
        'delete_transaction': 'حذف المعاملة',
        'transaction_list': 'قائمة المعاملات',
        'search_transaction': 'بحث عن معاملة...',
        'income_expense': 'الدخل/المصروفات',
        
        # الأزرار
        'save': 'حفظ',
        'cancel': 'إلغاء',
        'delete': 'حذف',
        'edit': 'تعديل',
        'close': 'إغلاق',
        'search': 'بحث',
        'filter': 'تصفية',
        'all': 'الكل',
        'back': 'رجوع',
        'view': 'عرض',
        'actions': 'إجراءات',
        'add': 'إضافة',
        'reset': 'إعادة تعيين',
        
        # الرسائل
        'confirm_delete': 'هل أنت متأكد من الحذف؟',
        'success': 'نجاح',
        'error': 'خطأ',
        'no_data': 'لا توجد بيانات',
        'loading': 'جاري التحميل...',
        
        # رسائل الإشعارات
        'transaction_deleted': 'تم حذف المعاملة',
        'transaction_updated': 'تم تحديث المعاملة',
        'transaction_added': 'تمت إضافة المعاملة',
        'customer_deleted': 'تم حذف العميل',
        'customer_updated': 'تم تحديث العميل',
        'customer_added': 'تمت إضافة العميل',
        'product_deleted': 'تم حذف المنتج',
        'product_updated': 'تم تحديث المنتج',
        'product_added': 'تمت إضافة المنتج',
        'product_removed': 'تمت إزالة المنتج',
        'receipt_deleted': 'تم حذف الفاتورة',
        'receipt_created': 'تم إنشاء الفاتورة بنجاح',
        'select_product_error': 'يرجى اختيار منتج',
        'invalid_quantity_error': 'يرجى إدخال كمية صحيحة',
        'invalid_price_error': 'يرجى إدخال سعر صحيح',
        'add_product_error': 'يرجى إضافة منتج واحد على الأقل',
        'error_occurred': 'حدث خطأ ما',
        'logout_success': 'تم تسجيل الخروج بنجاح.',
        'user_created': 'تم إنشاء المستخدم {username} بنجاح.',
        'password_reset': 'تم إعادة تعيين كلمة مرور المستخدم {username}.',
        
        # رسائل التأكيد
        'confirm_delete_transaction': 'هل أنت متأكد من حذف هذه المعاملة؟',
        'confirm_delete_customer': 'هل أنت متأكد من حذف هذا العميل؟',
        'confirm_delete_product': 'هل أنت متأكد من حذف هذا المنتج؟',
        'confirm_delete_receipt': 'هل أنت متأكد من حذف هذه الفاتورة؟ لا يمكن التراجع عن هذا الإجراء.',
        'confirm_clear_form': 'هل أنت متأكد من مسح النموذج؟',
        'confirm_delete_user': 'هل أنت متأكد من حذف المستخدم {username}؟ لا يمكن التراجع عن هذا الإجراء.',
        'delete_user': 'حذف المستخدم',
        'user_deleted': 'تم حذف المستخدم {username}',
        'cannot_delete_self': 'لا يمكنك حذف حسابك الخاص.',
        'admin_required': 'هذه العملية تتطلب صلاحيات المدير.',
        
        # تسجيل الدخول/المستخدم
        'login': 'تسجيل الدخول',
        'logout': 'تسجيل الخروج',
        'username': 'اسم المستخدم',
        'password': 'كلمة المرور',
        'email': 'البريد الإلكتروني',
        'remember_me': 'تذكرني',
        'admin': 'المشرف',
        'platform_admin': 'مدير المنصة',
        'observer': 'المشاهد',
        'admin_panel': 'لوحة التحكم',
        'platform_admin_panel': 'لوحة تحكم مدير المنصة',
        'company_admin_panel': 'لوحة تحكم مشرف الشركة',
        'user_management': 'إدارة المستخدمين',
        'users': 'المستخدمون',
        'role': 'الدور',
        'status': 'الحالة',
        'active': 'نشط',
        'inactive': 'غير نشط',
        'add_new_user': 'إضافة مستخدم جديد',
        'reset_password': 'إعادة تعيين كلمة المرور',
        'new_password': 'كلمة مرور جديدة',
        'cancel': 'إلغاء',
        'never': 'لم يسبق له الدخول',
        'you': 'أنت',
        'users_list': 'قائمة المستخدمين',
        'request_account': 'طلب حساب منشأة',
        'request_account_subtitle': 'أرسل طلبًا لإنشاء حساب منشأة جديد',
        'business_info': 'معلومات المنشأة',
        'business_type': 'نوع المنشأة',
        'select_option': 'اختر...',
        'cafe': 'مقهى',
        'market': 'ماركت',
        'restaurant': 'مطعم',
        'general': 'عام',
        'authorized_person': 'اسم المسؤول',
        'short_note': 'ملاحظة قصيرة',
        'short_note_placeholder': 'معلومات قصيرة عن المنشأة...',
        'submit_request': 'إرسال الطلب',
        'account_requests': 'طلبات الحسابات',
        'account_requests_subtitle': 'إدارة طلبات حسابات المنشآت',
        'request_list': 'قائمة الطلبات',
        'request_date': 'تاريخ الطلب',
        'pending': 'قيد الانتظار',
        'approved': 'تمت الموافقة',
        'rejected': 'مرفوض',
        'approve': 'موافقة',
        'reject': 'رفض',
        'no_requests': 'لا توجد طلبات بعد',
        'approve_request_title': 'الموافقة على الطلب',
        'approve_request_confirm': 'هل أنت متأكد من الموافقة على طلب {company}؟',
        'approve_request_info': 'عند الموافقة سيتم إنشاء سجل منشأة جديد ومستخدم مدير.',
        'reject_request_title': 'رفض الطلب',
        'rejection_reason': 'سبب الرفض',
        'rejection_reason_placeholder': 'يرجى كتابة سبب الرفض...',
        'request_already_processed': 'تمت معالجة هذا الطلب مسبقًا.',
        'temporary_password': 'كلمة المرور المؤقتة',
        'fill_required_fields': 'يرجى تعبئة الحقول المطلوبة.',
        'request_submitted': 'تم استلام طلب الحساب بنجاح.',
        'request_approved': 'تمت الموافقة على طلب الحساب.',
        'request_rejected': 'تم رفض طلب الحساب.',
        'login_credentials': 'بيانات تسجيل الدخول',
        
        # لوحة تحكم مدير المنصة
        'platform_admin_dashboard': 'لوحة تحكم مدير المنصة',
        'global_system_management': 'إدارة النظام العامة',
        'total_companies': 'إجمالي المنشآت',
        'total_company_users': 'مستخدمي الشركات',
        'platform_admins': 'مديرو المنصة',
        'pending_requests': 'الطلبات المعلقة',
        'recent_companies': 'آخر المنشآت المضافة',
        'quick_actions': 'إجراءات سريعة',
        'manage_companies': 'إدارة المنشآت',
        'all_companies': 'جميع المنشآت',
        'manage_all_companies': 'إدارة جميع المنشآت في النظام',
        'no_companies_yet': 'لا توجد منشآت بعد',
        'view_all': 'عرض الكل',
        'back': 'رجوع',
        'add_platform_admin': 'إضافة مدير منصة جديد',
        'platform_admin_management_desc': 'مديرو مستوى المنصة (مستقلون عن المنشآت)',
        'no_platform_admins': 'لا يوجد مديرو منصة بعد',
        
        # إدارة المنشآت
        'platform_management': 'إدارة المنصة',
        'suspended': 'معلّق',
        'suspend': 'تعليق',
        'activate': 'تفعيل',
        'delete': 'حذف',
        'view_details': 'عرض التفاصيل',
        'delete_company': 'حذف المنشأة',
        'delete_company_confirm': 'هل أنت متأكد من حذف المنشأة وجميع بياناتها؟',
        'delete_company_warning': 'لا يمكن التراجع عن هذا الإجراء! سيتم حذف جميع المستخدمين والعملاء والمعاملات والفواتير.',
        'company_details': 'تفاصيل المنشأة',
        'company_info': 'معلومات المنشأة',
        'company_users': 'مستخدمو المنشأة',
        'suspend_company': 'تعليق المنشأة',
        'activate_company': 'تفعيل المنشأة',
        'edit_company': 'تعديل المنشأة',
        'save': 'حفظ',
        'no_users': 'لا يوجد مستخدمون',
        'pending_requests_count': 'الطلبات المعلقة',
        'contact_info': 'معلومات الاتصال',
        'request_details': 'تفاصيل الطلب',
        'close': 'إغلاق',
        'delete_request': 'حذف الطلب',
        'delete_request_confirm': 'هل أنت متأكد من حذف هذا الطلب نهائيًا؟',
        'delete_request_warning': 'لا يمكن التراجع عن هذا الإجراء!',
        
        'supplier_added': 'تم إضافة المورد',
        'supplier_updated': 'تم تحديث المورد',
        'supplier_deleted': 'تم حذف المورد',
        
        # الموردين
        'suppliers': 'الموردين',
        'suppliers_subtitle': 'إدارة الموردين وتتبع الديون',
        'new_supplier': 'مورد جديد',
        'edit_supplier': 'تعديل المورد',
        'delete_supplier': 'حذف المورد',
        'supplier_name': 'اسم الشركة',
        'company_name': 'اسم الشركة',
        'contact_person': 'الشخص المسؤول',
        'total_suppliers': 'إجمالي الموردين',
        'debtor_suppliers': 'الموردين المديونين',
        'clear_suppliers': 'غير المديونين',
        'total_debt': 'إجمالي الدين',
        'search_supplier': 'البحث عن مورد...',
        'no_suppliers': 'لا يوجد موردين مسجلين بعد.',
        'tax_number': 'الرقم الضريبي',
        'address': 'العنوان',
        'passive': 'غير نشط',
        'confirm_delete_supplier': 'هل أنت متأكد من حذف هذا المورد؟',
        'supplier_info': 'معلومات المورد',
        'supplier_transactions': 'معاملات المورد',
        'total_purchases': 'إجمالي المشتريات',
        'total_payments': 'إجمالي المدفوعات',
        'remaining_debt': 'الدين المتبقي',
        
        # التقارير
        'reports': 'التقارير',
        'reports_subtitle': 'تقارير وتحليلات مفصلة لمنشأتك',
        'view_report': 'عرض التقرير',
        'profit_loss_report': 'تقرير الربح/الخسارة',
        'profit_loss_desc': 'تحليل الدخل والمصروفات والربح الصافي الشهري',
        'top_products_report': 'المنتجات الأكثر مبيعاً',
        'top_products_desc': 'المنتجات ذات أعلى أداء في المبيعات',
        'customer_debts_report': 'ديون العملاء',
        'customer_debts_desc': 'العملاء المديونين وحالة الرصيد',
        'stock_report': 'تقرير المخزون',
        'stock_report_desc': 'المخزون الحالي وتنبيهات المخزون المنخفض',
        'monthly_trend': 'الاتجاه الشهري (آخر 12 شهر)',
        'income_transactions': 'معاملات الدخل',
        'expense_transactions': 'معاملات المصروفات',
        'no_income_transactions': 'لا توجد معاملات دخل',
        'no_expense_transactions': 'لا توجد معاملات مصروفات',
        'total_sales_quantity': 'إجمالي كمية المبيعات',
        'sales_by_product': 'المبيعات حسب المنتج',
        'sales_by_unit': 'توزيع الوحدات',
        'top_selling_products': 'قائمة المنتجات الأكثر مبيعاً',
        'quantity_sold': 'الكمية المباعة',
        'no_sales_data': 'لا توجد بيانات مبيعات بعد',
        'total_debtors': 'العملاء المديونين',
        'remaining_debt': 'الدين المتبقي',
        'debt_distribution': 'توزيع الديون',
        'debt_vs_payment': 'الديون مقابل المدفوعات',
        'paid': 'مدفوع',
        'remaining': 'متبقي',
        'high_risk': 'خطر عالي',
        'medium_risk': 'خطر متوسط',
        'low_risk': 'خطر منخفض',
        'no_debt_customers': 'تم سداد ديون جميع العملاء',
        'low_stock_products': 'منتجات المخزون المنخفض',
        'total_stock_value': 'إجمالي قيمة المخزون',
        'low_stock_alert': 'تنبيه المخزون المنخفض!',
        'products_need_restock': 'منتجات تحتاج إلى تجديد المخزون',
        'view_low_stock': 'عرض المخزون المنخفض',
        'stock_details': 'تفاصيل المخزون',
        'current_stock': 'المخزون الحالي',
        'threshold': 'الحد الأدنى',
        'normal': 'عادي',
        'low': 'منخفض',
        'no_low_stock_products': 'لا توجد منتجات بمخزون منخفض',
        'filter': 'تصفية',
        'date': 'التاريخ',
        'description': 'الوصف',
        'revenue': 'الإيرادات',
        'status': 'الحالة',
        
        # صفحة المعاملات
        'this_month': 'هذا الشهر',
        
        # مفاتيح التقارير
        'net_profit': 'صافي الربح',
        'top_products_subtitle': 'المنتجات ذات أعلى أداء في المبيعات',
        'total_revenue': 'إجمالي الإيرادات',
        'customer_debts_subtitle': 'العملاء المديونين وحالة الرصيد',
        'total_paid': 'إجمالي المدفوع',
        'stock_report_subtitle': 'المخزون الحالي وتنبيهات المخزون المنخفض',
        'show_low_stock_only': 'عرض المخزون المنخفض فقط',
    }
}

class TranslationDict:
    """Dict wrapper that allows attribute access without conflicting with built-in methods"""
    def __init__(self, data):
        self._data = data
    
    def __getattr__(self, key):
        if key.startswith('_'):
            return object.__getattribute__(self, key)
        return self._data.get(key, key)
    
    def __getitem__(self, key):
        return self._data.get(key, key)
    
    def get(self, key, default=None):
        return self._data.get(key, default if default is not None else key)

def get_translation(key, lang='tr'):
    """Belirtilen dilde çeviriyi döndürür"""
    if lang not in TRANSLATIONS:
        lang = 'tr'
    return TRANSLATIONS.get(lang, {}).get(key, key)

def get_all_translations(lang='tr'):
    """Belirtilen dilin tüm çevirilerini döndürür"""
    if lang not in TRANSLATIONS:
        lang = 'tr'
    return TranslationDict(TRANSLATIONS.get(lang, TRANSLATIONS['tr']))
